...
export function AddTransaction({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-0 md:p-6">
      <div className="w-full h-full md:max-w-xl md:h-[700px] bg-[#09090B] border-0 md:border md:border-white/5 md:rounded-[2.5rem] flex flex-col text-zinc-100 font-sans md:shadow-2xl overflow-hidden relative">
        <header className="flex justify-between items-center p-6">
          <button onClick={onBack} className="p-2.5 -ml-2 bg-[#18181B] border border-white/5 rounded-full text-zinc-400 hover:text-zinc-100 transition-colors cursor-pointer">
            <ArrowLeft size={20} />
          </button>
          <div className="flex bg-[#18181B] border border-white/5 rounded-full p-1">
            <button
              onClick={() => { setType('expense'); setCategory(CATEGORIES.expense[0]); }}
              className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all cursor-pointer ${type === 'expense' ? 'bg-[#FF3366]/10 text-[#FF3366]' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
              Saída
            </button>
            <button
              onClick={() => { setType('income'); setCategory(CATEGORIES.income[0]); }}
              className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all cursor-pointer ${type === 'income' ? 'bg-brand-primary/10 text-brand-primary' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
              Entrada
            </button>
          </div>
          <button 
            ...
            className={`p-2.5 -mr-2 bg-[#18181B] border border-white/5 rounded-full transition-colors cursor-pointer ${isManual ? 'text-brand-primary' : 'text-zinc-400 hover:text-zinc-100'}`}
          >
            <Tag size={20} />
          </button>
        </header>
        ...
        <div className="flex-1 flex flex-col p-6 overflow-y-auto">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {['Uber', 'Ifood', 'Mercado', 'Almoço'].map(suggestion => (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-full bg-[#18181B] border border-white/5 text-xs font-medium text-zinc-400 hover:text-brand-primary hover:border-brand-primary/30 transition-all cursor-pointer"
                    >
                      {suggestion}
                    </button>
                  ))}
          ...
          <div className="mt-auto max-w-md mx-auto w-full pt-8">
            <button
              ...
              className={`w-full py-4 rounded-2xl font-medium text-sm transition-all active:scale-95 cursor-pointer ${
          ...
          {showCategories && (
            ...
            >
              <header className="flex justify-between items-center p-6">
                ...
                <button onClick={() => setShowCategories(false)} className="p-2.5 bg-[#18181B] border border-white/5 rounded-full text-zinc-400 hover:text-zinc-100 cursor-pointer">
                  <X size={20} />
                </button>
              </header>
              <div className="flex-1 overflow-y-auto p-6 grid grid-cols-2 gap-4 pb-32">
                {CATEGORIES[type].map(cat => (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-6 rounded-3xl transition-all border cursor-pointer ${
                      category === cat 
                        ? 'bg-black/60' 

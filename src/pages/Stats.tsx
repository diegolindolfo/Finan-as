  ...
  return (
    <div className="p-6 md:p-8 lg:p-12 max-w-md md:max-w-5xl lg:max-w-7xl mx-auto space-y-6 md:space-y-8 w-full">
      <header className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl md:text-4xl font-medium tracking-tight text-zinc-100">Categorias.</h1>
          <p className="text-xs md:text-sm text-zinc-400 font-medium mt-1 capitalize">{format(viewDate, 'MMMM yyyy', { locale: ptBR })}</p>
        </div>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Column: Month selector and general budget cap */}
        <div className="lg:col-span-5 space-y-6">
          <div className="flex justify-between items-center bg-[#18181B] border border-white/5 rounded-2xl p-3.5">
            <motion.button 
              whileTap={{ scale: 0.9 }}
              onClick={() => handleMonthChange(-1)} 
              className="p-2 text-zinc-400 hover:text-zinc-200 transition-colors bg-white/5 active:bg-white/10 rounded-xl cursor-pointer"
            >
              <ChevronLeft size={18} />
            </motion.button>
        ...
        {/* Right Column: Distribution and categories list */}
